...
class WebServer(Cog, name="Web server", description="HTTP serving and management."):
    def __init__(self, bot: Sersi, config: Configuration):
        ...
        def uptimerobot(request):
            return Response(text="Hewwo UwuptimeWowobowot!!!", status=200)
        ...
        async def youtube_verification(request: Request):
            if request.query is None or {"hub.challenge", "hub.mode", "hub.topic"} <= request.query.keys():
                return Response(text="Invalid PuSH query", status=400)

            if not request.query["hub.topic"].startswith("https://www.youtube.com/xml/feeds/videos.xml?channel_id="):
                return Response(text="Invalid topic", status=400)
            ...
            return Response(body=request.query["hub.challenge"].encode("utf8"), status=201)

        @routes.post("/youtube/*")
        async def youtube_notification(request: Request):
            if not request.has_body:
                return Response(text="No body given", status=400)
            ...
            for origin in origins:
                ...
                if origin["@href"].startswith("https://www.youtube.com/xml/feeds/videos.xml?channel_id="):
                    channel: TextChannel = self.bot.get_channel(self.config.channels.youtube)
                    ...
                    return Response(text="Topic ignored", status=204)

            return Response(text="Topic not found ", status=400)
    ...
    async def on_ready(self):
        ...
        site = TCPSite(runner, port=self.config.port)
        await site.start()
