...
class Voting(commands.Cog):
    ...
    async def process_votes(self):
        with db_session() as session:
            details_list: list[VoteDetails] = session.query(VoteDetails).filter_by(outcome=None).all()
            for details in details_list:
                ...
                votes: dict[str, int] = {vote: count for vote, count in (
                    ...
                    .group_by(VoteRecord.vote).all()
                )}

                diff = votes.get('yes', 0) - votes.get('no', 0)
                colour = None
                if votes.get('yes', 0) >= vote_type.threshold and diff >= vote_type.difference:
                    details.outcome = "Accepted"
                    colour = nextcord.Colour.brand_green()
                    self.bot.dispatch(vote_type.action, details)
                elif votes.get('no', 0) >= vote_type.threshold and -diff >= vote_type.difference:
    ...
    async def on_interaction(self, interaction: nextcord.Interaction):
        ...
        with db_session(interaction.user) as session:
            ...
            if vote_details is None:
                await interaction.response.send_message("This vote does not exist", ephemeral=True)
                return
            if vote_details.outcome is not None:
                await interaction.response.send_message("This vote has already been decided", ephemeral=True)
                return
...
class BallotModal(nextcord.ui.Modal):
    def __init__(self, vote_type: VoteType, details: VoteDetails, vote: str, on_vote: callable):
