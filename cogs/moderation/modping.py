...
class ModPing(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.channel.category.name in ["Important stuff", "Administration Centre", "The Staff Zone", "The Mod Zone", "The CET Zone", "Moderation Support"]:  # ignores certain channels on ASC, given by Juniper
            return
        ...
        if modmention_check(self.config, message.content):
            ...
            if len(updated_message.embeds[0].fields) < 5:  # If there are less than 5 fields that means there is no field for response
