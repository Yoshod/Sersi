...
class Blacklist(commands.Cog):
    ...
    async def cb_bluser_proceed(self, interaction):
        ...
        await interaction.message.edit(
            f"{self.sersisuccess} User added to blacklist.", embed=None, view=None
        )

        # LOGGING

        logging = nextcord.Embed(title="User added to Blacklist")
        logging.add_field(
    ...
    async def listblacklist(self, ctx):
        ...
        for entry in self.blacklist:
            ...
            if member is None:
                ...
                nicelist = (
                    nicelist + f"**{member}** ({member.id}): {self.blacklist[entry]}\n"
                )

        listembed = nextcord.Embed(
            title="Blacklisted Member List", description=nicelist
        )
    ...
    async def cb_blrmuser_proceed(self, interaction):
        ...
        await interaction.message.edit(
            f"{self.sersisuccess} User has been removed from blacklist.",
            embed=None,
            view=None,
        )
        ...
        logging = nextcord.Embed(title="User Removed from Blacklist")
        logging.add_field(
            name="Moderator:", value=interaction.user.mention, inline=False
        )
    ...
    @commands.command(
        aliases=["rmbl", "removeuserfromblacklist", "blrmuser", "blremoveuser"]
    )
    async def removefromblacklist(self, ctx, member: nextcord.Member):
        ...
        await ConfirmView(self.cb_blrmuser_proceed).send_as_reply(
    ...
    async def checkblacklist(self, ctx, member: nextcord.Member):
        ...
        if member.id in self.blacklist:
            ...
            await ctx.send(
                f"{self.sersisuccess} Member {member} not found on blacklist!"
            )
            return False
