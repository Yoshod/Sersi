...
async def create_log(guild: nextcord.Guild, log_type: str, **kwargs):
    """
    ...
    with guild_db_manager.get_session(guild.id) as session:
        logging_channel = (
            session.query(LoggingChannels).filter_by(log_type=log_type).first()
        )
            ...
            case "logging_setup_complete":
                embed = SersiEmbed(
                    title=lang_manager.get_string(
                        guild.id, "logging.setup_complete.title"
                    ),
                    description=lang_manager.get_string(
                        guild.id, "logging.setup_complete.description"
                    ).format(**kwargs),
                )
